...
public class ShopBaseinfoServiceImpl  implements ShopBaseinfoService{
	...
	public Map<String, Object> findById(String shopId) {
		String sql = "SELECT  i.id, i.shop_name shopName, (SELECT  a.account FROM t_sys_user a WHERE a.shop_id = i.id AND ISENABLED = '1' and ACCOUNT_TYPE = '4' ORDER BY create_dt LIMIT 0,1) shopAccountName ,"+
				...
				"   i.shop_category_id categoryId, i.show_in_first showInFirst, i.bank_id bankId, i.is_recommend isRecommend, i.logo shopLogo, i.show_pic showPic, brand_id brandId, i.zone_used/1024/1024/1024 usedSize, r.brand_name brandName, c.name categoryName "+
				"	FROM t_shop_info i LEFT JOIN  t_bank b ON i.bank_id =  b.id" +
				" left join t_shop_brand r on i.brand_id = r.id  left join t_shop_category c on i.shop_category_id = c.id where  i.id = ? ";
