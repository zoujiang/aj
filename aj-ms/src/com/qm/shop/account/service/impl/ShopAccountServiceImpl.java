...
public class ShopAccountServiceImpl  implements ShopAccountService{
	...
	public DataModel<Map<String, Object>> getList(ShopAccountVO limitKey) {
		...
		String sql = "select  a.id, i.shop_name shopName, account username, a.tele tel, a.name nickname, DATE_FORMAT(a.create_dt,'%Y-%m-%d %H:%i:%s') createTime, a.ISENABLED status from t_sys_user a, t_shop_info i WHERE a.shop_id = i.id and a.ACCOUNT_TYPE = '4' and a.shop_id = ?";
		String countSql = "select count(1) from t_sys_user a, t_shop_info i WHERE a.shop_id = i.id and a.ACCOUNT_TYPE = '4'  and a.shop_id = ?";
		...
		if(limitKey != null){
			...
			if(limitKey.getUsername() != null && !"".equals(limitKey.getUsername().toString())){
				sql += " and a.account like concat ('%',?,'%')";
				countSql += " and a.account like concat ('%',?,'%')";
