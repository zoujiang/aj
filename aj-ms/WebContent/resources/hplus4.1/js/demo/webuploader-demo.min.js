jQuery(function() {
	...
	function t(e) {
		...
		if (e !== k) {
			...
			case "confirm":
				...
				break;
			case "finish":
				$("#pickerImgCount").val(0),
				a = n.getStats(),
				//a.successNum ? alert("上传成功") : (k = "done", location.reload())
				a.successNum ? console.info("上传成功") : (k = "done", location.reload())
			}
			i()
		}
	}
	...
	n = WebUploader.create({
		...
		disableGlobalDnd: !0,
		chunked: !0,
		server: "/ms/admin/fileUploadCommon/f/fileUpload?from=dynamic",
		fileNumLimit: 12,
		compress:false
	...
	n.onFileQueued = function(a) {
		var hadFile = $("#oldPhotoUrls").val();
		var hadFileSize = 0;
		if(hadFile != null && hadFile != ""){
			hadFileSize = hadFile.split(",").length;
		}
		if(hadFileSize + m >=12){
			console.info("动感影集单个相册最多可以上传12张照片");
		}else{
	...
	n.onUploadSuccess = function (file, respone) {
       //$('#' + file.id).find('p.state').text('已上传');
        var photoUrls = $("#photoUrls").val();
        if(photoUrls != ""){
        	$("#photoUrls").val(photoUrls+","+respone.url);
        }else{
        	$("#photoUrls").val(respone.url);
        }
