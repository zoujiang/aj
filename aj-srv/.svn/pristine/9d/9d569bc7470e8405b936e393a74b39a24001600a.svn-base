...
@Service("taskAccept")
public class AcceptTaskService implements PublishService {
	...
	public Object publishService(Object obj) throws PublicException {
		...
		if(params.optString("status") == null || "".equals(params.optString("status"))){
			returnJSON.put("returnCode", Constant.RETURNCODE_FAILED);
			returnJSON.put("result", result);
			returnJSON.put("errorMsg", "是否接受任务不能为空！");
			return returnJSON.toString();
		}
		
		try {
			//params.put("status", 2);
			taskService.modifyAccept(params);
			result.put("succMsg", "2".equals(params.optString("status"))?"接受任务成功！":"拒绝任务成功！");	
			...
			returnJSON.put("result", result);
			returnJSON.put("errorMsg", "2".equals(params.optString("status"))?"接受任务失败！":"拒绝任务失败！");
			return returnJSON.toString();
		}
	}
