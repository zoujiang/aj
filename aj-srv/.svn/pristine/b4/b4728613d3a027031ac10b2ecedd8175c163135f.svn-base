...
public class ThreadPool extends ThreadGroup{
	...
	private boolean isClosed = false;  //线程池是否关闭    
	 ...
	 public ThreadPool(int poolSize) {  //poolSize 表示线程池中的工作线程的数量   
		...
		log.debug("ThreadPool("+ poolSize + ") - start"); //$NON-NLS-1$
	     setDaemon(true);               //继承到的方法，设置是否守护线程池   
	     workQueue = new LinkedList();  //创建工作队列   
	     for(int i = 0; i < poolSize; i++) {   
	         new WorkThread(i).start();   //创建并启动工作线程,线程池数量是多少就创建多少个工作线程   
	     }   
	     log.debug("ThreadPool("+ poolSize + ") - end"); //$NON-NLS-1$
	     log.info("线程处理池初始化完毕，线程个数"+poolSize+"个");
	 }   
	    
	 /** 向工作队列中加入一个新任务,由工作线程去执行该任务*/  
	 public synchronized void execute(Runnable task) {   
		 log.debug("execute("+ task + ") - start"); //$NON-NLS-1$
	     if(isClosed) {   
	     ...
	     log.debug("execute("+ task + ") - end"); //$NON-NLS-1$
	 }   
	 ...
	 private synchronized Runnable getTask(int threadid) throws InterruptedException {   
	     while(workQueue.size() == 0) {   
	         ...
	         log.info("工作线程"+threadid+"等待任务..."); //$NON-NLS-1$
	         wait();             //如果工作队列中没有任务,就等待任务   
	     }   
	     log.info("工作线程"+threadid+"开始执行任务..."); //$NON-NLS-1$
	     return (Runnable) workQueue.removeFirst(); //反回队列中第一个元素,并从队列中删除   
	 }   
	    
	 /** 关闭线程池 */  
	 public synchronized void closePool() {  
			log.debug("closePool() - start"); //$NON-NLS-1$
	     ...
	     log.debug("closePool() - end"); //$NON-NLS-1$
	 }   
	    
	 /** 等待工作线程把所有任务执行完毕*/  
	 public void waitFinish() {   
		 log.debug("waitFinish() - start"); //$NON-NLS-1$
	     synchronized (this) {   
	         isClosed = true;   
	     ...
	     log.debug("waitFinish() - end"); //$NON-NLS-1$
	 }   
