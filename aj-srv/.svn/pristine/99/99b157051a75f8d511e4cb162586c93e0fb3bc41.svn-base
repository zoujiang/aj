...
@Service("easemobPwdMod")
public class ModEasemobPwdService implements PublishService {
	@Autowired EasemobUserService easemobUserService;

	private Logger log = Logger.getLogger(ModEasemobPwdService.class);
	@Override
	public Object publishService(Object obj) throws PublicException {
		...
		if(params.optString("newpassword") == null || "".equals(params.optString("newpassword"))){
			returnJSON.put("returnCode", Constant.RETURNCODE_FAILED);
			returnJSON.put("result", result);
			returnJSON.put("errorMsg", "环信新密码不能为空！");
			return returnJSON.toString();
		}
		try {
			JSONObject object = easemobUserService.modPwd(params);
			log.info(object.toString());
			if("set user password".equals(object.get("action"))){
				//JSONArray array = object.getJSONArray("entities");
				result.put("succMsg", "修改环信密码成功！");	
				...
				if(object.get("statusCode") != null){
					...
					returnJSON.put("errorMsg", "修改环信密码失败！");
				}
				return returnJSON.toString();
			}
