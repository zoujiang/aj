...
@Service("easemobFriendAdd")
public class AddEasemobFriendService implements PublishService {
	
	@Autowired EasemobUserService easemobUserService;

	private Logger log = Logger.getLogger(AddEasemobFriendService.class);
	...
	public Object publishService(Object obj) throws PublicException {
		...
		if(params.optString("friendUserName") == null || "".equals(params.optString("friendUserName"))){
			returnJSON.put("returnCode", Constant.RETURNCODE_FAILED);
			returnJSON.put("result", result);
			returnJSON.put("errorMsg", "要添加好友的用户环信ID不能为空！");
			return returnJSON.toString();
		}
		try {
			JSONObject object = easemobUserService.addFriend(params);
			log.info(object.toString());
			if(object.get("statusCode") != null && "200".equals(object.get("statusCode").toString())){
				JSONArray array = object.getJSONArray("entities");
				result.put("succMsg", "添加好友关系到环信成功！");	
				...
				if(msg != null && !"".equals(msg)){
					...
					returnJSON.put("errorMsg", "添加好友关系到环信失败！");
				}
