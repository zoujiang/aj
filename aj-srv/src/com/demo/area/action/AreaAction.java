...
public class AreaAction extends FtpImgDownUploadAction {
	...
	public String addArea(AreaMgrVo areaMgrVo){
		try {
			if(StringUtils.isNotEmpty(areaMgrVo.getAreaPicFile().getOriginalFilename())){
	...
	@ResponseBody
	public DataModel<AreaMgrVo> queryAreaList(String req, String page, String rows, String sort, String order) {
		log.debug("queryAreaList(req:" + req +",rows:"+rows+",sort:"+sort+ ",order:"+order+") - start");
		...
		log.debug("queryAreaList("+searchText+","+page+","+rows+","+sort+","+order+") - end");
		return areaList;
	}
	/**
	 * 区域资源数据导出
	...
	public String areaExport(String req, String page, String rows, String sort, String order, String columns, HttpServletRequest request) {
		...
		try {
			incStatList = areaServiceImpl.queryAreaList(key);
		} catch (ServiceException e) {
			e.printStackTrace();
		}
		...
		request.setAttribute(ExcelWriteHelperPoi.EXPORT_FILENAME, "ResOrder");
