...
<head>
    ...
    function AppMgr(){
		...
		AppMgr.manage = function(id,type,e){
			layer_show("管理学生荣誉", getProjectName() +"/pages/qm/kindergarten/honor/manage.jsp?id="+id+"&type="+type,"930","500");
		}
		AppMgr.add = function(id, valid,e){
			layer_show("上传荣誉", getProjectName() +"/pages/qm/kindergarten/honor/add.jsp?type=2&ownerId="+id,"850","400");
		};
		AppMgr.download = function(id, type,e){
			layer_show("下载", getProjectName() +"/pages/qm/kindergarten/honor/downloadlist.jsp?type=2&ownerId="+id,"850","400");
		};
    	this.initDatas = function(){
    		...
    		$.ajax({
			 ...
			 $table.bootstrapTable({
    			url: getProjectName() + "/admin/kindergarten/honor/list",
                showExport:false,
                toolbar:'#toolbar',
               // exportDataType:"selected",
    		    ...
    		    columns: [/*{
    		        ...
    		        formatter : function(value, row, index){
    		        	...
    		        	var manage="<span  height = '23px'  width = '23px'><a style='color:blue;' href='#' onclick=\"AppMgr.manage('" + row.id + "','2',event)\" style='cursor: pointer' >管理&nbsp;</a></span>";
    		        		
    		        	var	 add="<span  height = '23px'  width = '23px'><a style='color:blue;' href='#' onclick=\"AppMgr.add('" + row.id + "','0',event)\" style='cursor: pointer' >上传&nbsp;</a></span>";
    		        	return manage +  add;
    		        },
    		        width: 200
    		    }]
    ...
    function viewTeamHonor(){
    	...
    	if(gradeId == null || gradeId == ''){
    		...
    		layer_show("管理集体荣誉", getProjectName() +"/pages/qm/kindergarten/honor/manage.jsp?id="+gradeId+"&type=1","930","500");
    	}
    }
    function addTeamHonor(){
    	var gradeId = $("#gradeId").val();
    	if(gradeId == null || gradeId == ''){
    		layer.msg("请先选择班级", {title:'提示', btn: ['确定'],icon: 6}, function(index){
				parent.layer.close(index);
    		...
    		layer_show("上传集体荣誉", getProjectName() +"/pages/qm/kindergarten/honor/add.jsp?type=1&ownerId="+gradeId,"930","500");
    	}
    }
    </script>
</head>
                    ...
                    <div class="form-inline" style="width: 880px;float: left;">
                    	...
                    	班级：<select id="gradeId" name="gradeId" class="form-control input-inline"  width="280px">
                         ...
                         学生姓名：<input type="text" placeholder="学生姓名" class="form-control input-inline"  id="name" name="name" width="280px">
                           ...
                           <button  id="btn_search" type="button" class="btn btn-inline btn-default" style="margin-top: 4px;" onclick="addTeamHonor()">上传集体荣誉</button> 
                           <button  id="btn_search" type="button" class="btn btn-inline btn-default" style="margin-top: 4px;" onclick="viewTeamHonor()">查看集体荣誉</button> 
                    </div>
                </div>
