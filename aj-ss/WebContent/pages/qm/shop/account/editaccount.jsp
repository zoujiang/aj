	...
	<script type="text/javascript">
	 function AppMgr(){
		 ...
		 this.initDatas = function(){
			 ...
			 $.ajax({
	             type: "GET",
	             url: "<%=path %>/admin/shop/account/find",
	             data: "id=<%=id %>",
	             dataType: "json",
	             success: function(data){
	                  if(data.success){
	                	  shopId = data.message.shopId;
	                	  $("#username").val(data.message.username);
	                	  $("#shopId").val(data.message.shopId);
	            	 ...
	            	 var html = "";
	            	  ...
	            	  { 
	            		  if(content.id == shopId){
	            		  		html += "<option selected='selected' value='"+content.id+"'>"+content.shopName+"</option>";
	            		  }else{
	 ...
	 function saveUser(){
		 ...
		 var shopId = $("#shopId").val();
			...
			$("#userForm").ajaxSubmit({
			     type: "post",
			     url: "<%=path %>/admin/shop/account/update",
			     dataType: "json",
	 ...
	 function existAccount(username){
		 ...
		 $.ajax({
             type: "POST",
             url: "<%=path %>/admin/shop/account/findByUsername",
               	 	...
               	 	<form class="form-horizontal" id="userForm"  method="post">
						    ...
						    <div class="form-group col-sm-6">
	                            <label class="col-sm-4 control-label">状态：</label>
	                             <div class="col-sm-8">
	               					<input type="radio" id="ra0" name="status" value="1" checked="checked">正常&nbsp;&nbsp;&nbsp;&nbsp;
	               					<input type="radio" id="ra1" name="status" value="2">冻结
	               				</div>
	                        </div>
                        </div>
                        <div class="row">
