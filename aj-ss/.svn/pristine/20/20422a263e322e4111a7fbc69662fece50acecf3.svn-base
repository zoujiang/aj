...
public class ShopPayAbleServiceImpl  implements ShopPayAbleService{
	...
	public int saveOrUpdate(String id, String accountZFB, String secretZFB,
			...
			String updateTime, String serviceAmount) {
		
		String querySql = "delete from t_shop_pay_able where id = ?" ;
		baseDAO.execteNativeBulk(querySql, new Object[]{id});
		//新增
		String sql = "insert into t_shop_pay_able (id, account_zfb, secret_zfb,account_wx,secret_wx, pay_type_android, pay_type_ios, access_android_zfb, access_android_wx, access_ios_zfb, access_ios_wx, create_time, update_time,service_amount) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
		int index = baseDAO.execteNativeBulk(sql, id, accountZFB, secretZFB, accountWX, secretWX, payTypeAndroid, payTypeIOS, accessAndroidZFB, accessAndroidWX, accessIOSZFB, accessIOSWX, createTime, updateTime, serviceAmount);
