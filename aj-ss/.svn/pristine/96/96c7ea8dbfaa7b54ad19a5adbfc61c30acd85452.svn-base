...
public class ShopAlbumServiceImpl  implements ShopAlbumService{
	...
	public DataModel<Map<String, Object>> getList(ShopAlbumVO limitKey) {
		...
		String sql = "SELECT a.id, u.user_tel userTel, a.album_name albumName, (SELECT COUNT(1) FROM t_shop_photo p WHERE p.album_id = a.id ) photoCount, a.is_pay isPay, a.pay_type payType, a.create_time createTime, a.download_address downloadAddress, a.download_secret downloadSecret,o.pay_type buyType,o.transaction_id transactionId, a.view_count viewCount, a.photo_time photoTime, i.shop_name shopName,a.original_price originalPrice,o.total_fee totalFee, o.status payStatus, a.pay_time payTime FROM t_shop_album a LEFT JOIN t_shop_album_order o ON a.id = o.album_id and o.status = 1, t_shop_info i, t_shop_customer_user u WHERE a.shop_id = i.id  AND a.user_id = u.id ";
	...
	public int save(ShopAlbumVO vo) {
		String sql = "INSERT INTO t_shop_album (id, shop_id, user_id, album_name, album_logo, is_pay, original_price, pay_type, create_time, download_address, download_secret, album_size, photo_time) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)";
		String sp = "|";
		String payType = vo.getPayType_3() + sp 
						...
						+ vo.getPayType_24() + sp
						+ vo.getPayType_25();
		return baseDAO.execteNativeBulk(sql, vo.getId(),vo.getShopId(), vo.getUserId(), vo.getAlbumName(), vo.getAlbumLogo(), vo.getAlbumType(), vo.getOrigPrice(), payType, vo.getCreateTime(), vo.getDownloadAddress(), vo.getDownloadSecret(), vo.getAlbumSize(), vo.getPhotoTime());
	}
	...
	public Map<String, Object> find(String id) {
		...
		String sql = "SELECT a.id, a.user_id userId, a.shop_id shopId, u.user_tel userTel, a.album_name albumName, a.album_logo albumLogo, a.original_price originalPrice, a.is_pay isPay, a.pay_type payType, a.download_address downloadAddress, a.download_secret downloadSecret, a.photo_time photoTime FROM t_shop_album a, t_shop_info i, t_shop_customer_user u WHERE a.shop_id = i.id  AND a.user_id = u.id and a.id = ?";
	...
	public int update(ShopAlbumVO vo) {
		...
		String sql = "update t_shop_album set shop_id =?, user_id=?, album_name=?, album_logo=?, is_pay=?, original_price=?, pay_type=?, download_address=?, download_secret =?, photo_time = ? where id = ?";
		String sp = "|";
		int i = 0;
		String payType = vo.getPayType_3() + sp 
						+ vo.getPayType_6() + sp 
