...
public class ExportExcelUtils {
	 ...
	 * @param out
	 */
	public HSSFWorkbook exportExcel2(List<Map<String, Object>> dataList, String headerColumn, String fileName) {
		HSSFWorkbook workbook = new HSSFWorkbook();// 声明一个工作薄
		HSSFSheet sheet = workbook.createSheet(fileName);// 生成一个表格
		sheet.setDefaultColumnWidth((short) 15);// 设置表格默认列宽度为15个字节
		...
		int index = 0;
		HSSFRow row0 = sheet.createRow(index);
		JSONArray ja = JSONArray.fromObject(headerColumn);
		columnLength = ja.size();
		columns = new String[columnLength];
		for (int column = 0; column < ja.size(); column++) {
			HSSFCell cell = row0.createCell(column);// 产生表格标题行
			cell.setCellStyle(headerStyle);
			JSONObject jo = (JSONObject) ja.get(column);
			String key = jo.keySet().iterator().next().toString();
			columns[column] = key;
			HSSFRichTextString text = new HSSFRichTextString(jo.getString(key));
			cell.setCellValue(text);
		}
		// 遍历集合数据，产生数据行
